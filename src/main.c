...
static void init(void) {
	...
	//set resource handle
	rh = resource_get_handle(RESOURCE_ID_STORY);
	//set filesize
...
//	int start_point = 0;
	...
	while(index < filesize){
		uint8_t *i_buffer = (uint8_t*)malloc(count);
		
		resource_load_byte_range(rh, index, i_buffer, count);
		...
		for(int i = 0; i < count; i++){
			...
			if(i_buffer[i] == 10){
				...
				size_t buffer_size = 8 * i;
				char *buffer = malloc( buffer_size );
				
				memcpy(buffer, i_buffer, i-2);
				APP_LOG(APP_LOG_LEVEL_DEBUG, "line %d: %s", counter, buffer);
			...
			if(i == count -1){
				APP_LOG(APP_LOG_LEVEL_DEBUG, "no break found");	
			}
...
//		start_point = end_point +2;
	...
	*/
	...
	uint8_t *s_buffer = (uint8_t*)malloc(count);
	resource_load_byte_range(rh, 0, s_buffer, count);
	...
	for(int i = 0; i < count; i++){
		if(s_buffer[i] == 10){
			APP_LOG(APP_LOG_LEVEL_DEBUG, "New line");
			//strncpy(buffer, s_buffer, i);
			memcpy(buffer, s_buffer, i-2);
			APP_LOG(APP_LOG_LEVEL_DEBUG, "line: %s", buffer);
		}
		APP_LOG(APP_LOG_LEVEL_DEBUG, "Index %d -> %d", i, s_buffer[i]);
	}
	
	//APP_LOG(APP_LOG_LEVEL_DEBUG, "Text1: %s", buffer);
	APP_LOG(APP_LOG_LEVEL_DEBUG, "Text1: %s", buffer);
*/	
